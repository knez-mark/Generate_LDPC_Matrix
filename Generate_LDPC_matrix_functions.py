...
def greedy_upper_triangulation(H, verbose=0):
    ...
    if np.linalg.matrix_rank(H_t) != H_t.shape[0]:
        print('Rank of H:', np.linalg.matrix_rank(tempArray))
    ...
    while t != (n - k - g):
        ...
        minResidualDegrees = np.zeros((1, numCols), dtype=int)

        for colNum in np.arange(numCols):
            nonZeroElements = np.array(H_residual[:, colNum].nonzero())
    ...
    temp1 = np.dot(E, invTmod2array) % 2
    temp2 = np.dot(temp1, A) % 2
    phi = (C - temp2) % 2
    if phi.any():
        try:
